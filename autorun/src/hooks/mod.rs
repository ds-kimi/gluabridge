		...
		if engine.IsConnected() {
			...
			if let Some(net) = unsafe { net.as_mut() } {
				...
				if curtime < CONNECTED.load(Ordering::Relaxed) {
					...
					#[cfg(ipc)]
					{
						crate::ipc::push_event(crate::ipc::Event::Connected {
							ip: unsafe { CStr::from_ptr(ip) }.to_string_lossy().to_string(),
						});

						// The lua state is fresh after a map change, so the
						// capture wrappers are gone; queue a re-attach.
						crate::ipc::console::reattach_if_active();
					}
				}
...
extern "fastcall" fn paint_traverse_h(
	...
	// Runs on gmod's own thread, so this is where deferred lua-state work lands.
	#[cfg(ipc)]
	crate::ipc::console::poll_attach();
		...
		if !queue.is_empty() {
			...
			match lua::get_state(realm) {
				Ok(state) => match lua::dostring(state, &script) {
					Err(why) => {
						error!("{why}");
						report_run(false, why.to_string());
					}
					Ok(_) => {
						info!("Script of len #{} ran successfully.", script.len());
						report_run(true, format!("ran {} bytes in {realm}", script.len()));
					}
				},
				Err(why) => {
					error!("{why}");
...
/// Mirrors executor results back to any attached editor.
#[cfg(executor)]
fn report_run(ok: bool, message: String) {
	#[cfg(ipc)]
	crate::ipc::push_event(crate::ipc::Event::RunResult { ok, message });
